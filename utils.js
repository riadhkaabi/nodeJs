...
const parseTimeToDate = (time)=>{
    ...
    time = (time - minute) / 60;
    const hour = (time % 24)+2;
    time = (time - hour) / 24;
    const day = time % 31 + 2;
    time = (time - (day - 1)) / 31;
    const month = time % 12 +1;
    time = (time - month) / 12;
    const year = time + 2001;
...
module.exports.decodeUserData72 = (userData)=>{
    const user = {
        id: userData.readUIntLE(0, 2),
        empRole: userData.readUIntLE(2, 1),
        empPwd: userData
          .subarray(3, 3+8)
          .toString('ascii')
          .split('\0')
          .shift(),
        empFirstname: userData
          .slice(11)
          .toString('ascii')
          .split('\0')
          .shift(),
...
module.exports.decodeRecordData40 = (recordData)=>{
    const record = {
        ...
        .split('\0')
        .shift(),
        punchTime: parseTimeToDate(recordData.readUInt32LE(27)),
        workstate : recordData.readUIntLE(31,1),
